...
@export var range_max_in_dir: float = 100.0 #can change these values
...
func _tick(_delta: float)-> Status:
	...
	print(dir,'   ',pos.x, ' agent pos:', agent.global_position.x)
	
	return SUCCESS
	
func rando_pos(dir):
	var vector: Vector2
	var distance = float(randi_range(range_min_in_dir, range_max_in_dir) * dir)
